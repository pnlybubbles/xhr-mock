...
/**
 * A constant for through to real xhr
 */
MockXMLHttpRequest.THROUGH_TO_REAL_XHR = {};
...
MockXMLHttpRequest.prototype.send = function(data) {
  ...
  self._sendTimeout = setTimeout(function() {
    ...
    } else if (response === MockXMLHttpRequest.THROUGH_TO_REAL_XHR) {
      realXhr = new real();
      realXhr.open(self.method, self.url, self.async, self.user, self.password);
      realXhr.addEventListener('load', function() {
        self.status             = realXhr.status;
        self._responseHeaders   = realXhr.getAllResponseHeaders();
        self.responseType       = realXhr.responseType;
